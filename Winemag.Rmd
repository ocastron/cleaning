...
L'únic tipus que haurem de canviar és el de points, ja que és una variable quantitativa, encara que estigui representada per valors numérics sencers.
...
Per tant, després d'examinar les varietats actuals durem a terme els canvis següents:
...
Uns altres grups que podriem analitzar seria puntuació i preus per pais o qulitat per pais.
...
```{r 3.2}
# comprovem la normalitat de la variància amb el test de Lavene per a la qualitat/preu
with(wine.final, tapply(price, points, var, na.rm=TRUE))
leveneTest(price~points, data = wine.final, center = "median")
```
...
No normalitzarem, atés que ens interesen els preus tal com s'ens presenten.
...
write.csv(wine.final, file = "./csv/winefinal.csv", row.names = FALSE)
```
